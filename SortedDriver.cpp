...
unmatched(list<string> & A, list<string> & B)
{
	...
	for (list<string>::iterator itA = A.begin(), itB = B.begin(); itA != A.end(); itB++) // For every item in A...
	{
		if (itB == B.end())			// Check if iterator B is at its end
		{
				itB = B.begin();	// Reset B iterator
				itA++;				// Increment iterator A
		}
		else if (itA != A.end())	// Check to make sure that iterator A is not at its end
		{
			// The way this loop is set up, I need to make sure to check the first element of B.
			// Otherwise, the first element of B is skipped after the first loop.
			if (*itA == *B.begin())	// Check if the current element of A is equal to the first element of B
			{
				inB++;				// Increment count of items that occur in A and B
				itA++;				// Increment iterator A
				itB = B.begin();	// Reset iterator B
			}
			else if (*itA == *itB)	// Check if the current element of A is equal to the current element of B
			{
				inB++;				// Increment count of items that occur in A and B
				itA++;				// Increment iterator A
				itB = B.begin();	// Reset iterator B. When the loop restarts, iterator B points to second element. Code is built around that knowledge.
			}
		}
	}
